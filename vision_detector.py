#!/usr/bin/env python3
"""
IoT Smart Fan Control System - Computer Vision Module
...
class VisionDetector:
    def __init__(self, config_file: str = 'vision_config.json'):
        ...
        # Skip serial connection - backend handles Arduino communication
        logger.info("Vision detector will communicate via backend API only")
    ...
    def send_to_arduino(self, command: str):
        """Send command to Arduino via serial - DISABLED, using backend API instead."""
        # Serial communication handled by backend server
        logger.debug(f"Detection command (sent via API): {command}")
        pass
    
    def send_to_dashboard(self, data: dict):
        """Send detection status to dashboard via HTTP."""
        try:
            logger.info(f"Attempting POST to {self.config['dashboard_url']}/api/vision-update")
            ...
            if response.status_code == 200:
                logger.info(f"Vision update sent successfully")
            else:
                logger.warning(f"Vision update failed with status {response.status_code}")
        except requests.exceptions.RequestException as e:
            # Dashboard might not be running
            logger.error(f"Failed to send vision update: {e}")
    
    def read_arduino_status(self):
        """Read status messages from Arduino - DISABLED, using backend API instead."""
        # Arduino communication handled by backend server
        return None
    ...
    def run(self):
        ...
        try:
            while self.running:
                logger.info(f"Loop iteration {frame_count}")
                ...
                if self.current_mode == 'AUTO':
                    ...
                    if detected:
                        ...
                        logger.info(f"Human detected! Confidence: {confidence:.2f}, Face size: {face_size}px")
                    else:
                ...
                if current_time - last_status_send >= 0.5:
                    status_data = {
                        'humanDetected': bool(self.human_detected),
                        'confidence': float(self.detection_confidence),
                        'timestamp': int(time.time() * 1000)
                    }
                    logger.info(f"Sending vision update to backend: {status_data}")
